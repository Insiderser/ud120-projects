...
print(min(filter(lambda x: x[0] > 0, finance_features), key= lambda x: x[0])[0])
print(max(filter(lambda x: x[0] > 0, finance_features), key= lambda x: x[0])[0])

### cluster here; create predictions of the cluster labels
### for the data and store them to a list called pred

k_means = KMeans(n_clusters=2, n_jobs=cpu_count())
